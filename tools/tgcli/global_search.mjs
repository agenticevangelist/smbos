...
const SEEN_FILE = '/tmp/gs_seen.json';
const LOG_FILE = '/tmp/gs.log';
...
// Фильтр нерелевантных стеков и платформ
...
const QUERIES = [
  ...
  'кто делает на tilda',
  'нужен tilda мастер',
  'сделать сайт на тильда',
  'нужен nocode разработчик',
  'нужен bubble разработчик',
  'нужен webflow разработчик',
  // AI / автоматизация
  'нужен ai агент',
  'автоматизировать бизнес процесс',
  'нужен чат бот для поддержки',
  'нужен голосовой бот',
  'ищу разработчика ai',
  'нужен интегратор crm',
  // Мобильные приложения
  'нужно мобильное приложение',
  'нужен разработчик react native',
  'нужен ios android разработчик',
  // Грузия / международные
  'developer needed georgia',
  'looking for developer tbilisi',
  'нужен разработчик тбилиси',
  'ищу разработчика грузия',
  // Фриланс биржи с запросами
  'нужен фрилансер разработчик',
  'ищу подрядчика разработка',
  'нужен разработчик удаленно срочно',
  // Инфобиз / эксперты
  'нужен бот для курса',
  'нужна воронка telegram',
  'хочу автоматизировать продажи',
  'нужен бот для онлайн школы',
  'настроить бот для курса',
  'сделать mini app telegram',
  'нужен разработчик mini app',
  'разработать telegram mini app',
  'нужен бот для записи клиентов',
  // Dubai / ОАЭ
  'need developer dubai',
  'looking for developer uae',
  'нужен разработчик дубай',
  'ищу разработчика дубай оаэ',
  'website developer needed dubai',
  'telegram bot developer dubai',
  'crm for business dubai',
  // Стамбул / Турция
  'нужен разработчик стамбул турция',
  'ищу разработчика турция',
  'need developer istanbul',
  // Недвижимость CRM
  'нужна crm для агентства недвижимости',
  'crm для риелторов',
  'автоматизация для агентства недвижимости',
  'бот для агентства недвижимости',
  // Delivery запросы
  'подключить wolt',
...
function scoreMsg(text) {
  ...
  if (SKIP_STACK_RE.test(text)) return 0; // Tilda, WordPress, .NET и прочий нерелевантный стек
  ...
  for (let qi = 0; qi < QUERIES.length; qi++) {
    ...
    try {
      ...
      for (const msg of (res.messages || [])) {
        ...
        if (!fromId) continue;
        if (fromId === '8466294883') continue; // пропускаем собственный аккаунт
