...
1. **OpenClaw is the core.** All intelligence, search, notifications, scheduling, and inter-service communication goes through OpenClaw agents. SMBOS is a UI shell — it renders data and sends user actions to agents. Don't build standalone frontend logic for things an agent should handle.
...
Scheduling uses OpenClaw's built-in cron system (`cron` tool + RPC: `cron.add`, `cron.list`, etc.). No SMBOS-side cron engine.
Chat history stored server-side in OpenClaw JSONL sessions, loaded via WS RPC `sessions.history`. localStorage used as cache only.
SMBOS accesses OpenClaw via WebSocket (port 18789) proxied through `/api/openclaw/*` routes — never reads OpenClaw files directly.
OpenClaw config: `~/.openclaw/openclaw.json` (hot-reloaded). SMBOS manages it via RPC `config.patch` + `config.apply`.
