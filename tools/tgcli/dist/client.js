...
const STORE_DIR = path.join(process.env.TGCLI_STORE || path.join(import.meta.dirname, '..', 'store'));
...
export async function listChats(limit = 100, query) {
    const c = await connect();
...
export async function searchChats(query, limit = 20) {
    const c = await connect();
    const result = await c.invoke(new Api.contacts.Search({ q: query, limit }));
    const results = [];
